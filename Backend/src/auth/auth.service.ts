...
import { EventCoordinatorService } from "src/event-coordinator/event-coordinator.service";
...
export class AuthService {
  ...
  //       user = await this.eventCoordinatorService.findByCredentials(loginDto);
  //       if (user && await bcrypt.compare(loginDto.password, user.password)) {
  //         const { password, ...result } = user;
  //         return result;
  //       }  
  ...
  async validateUser(role: string, loginDto: AuthDTO): Promise<any> {
    const services = {
      ...
      'judge': this.judgeService,
      'entrepreneur': this.entrepreneurService,
      'event-coordinator': this.eventCoordinatorService,
    };
    ...
    const user = await service.findByCredentials(loginDto);
    if (!user) {
  ...
  async changePassword(role: string, id: number, changePasswordDto: changePasswordDTO): Promise<string> {
    ...
    return await service.changeAdminPassword(id, changePasswordDto);
    ...
    // } else if (role === 'event-coordinator') {
    //   return await this.eventCoordinatorService.changeEventCoordinatorPassword(id, changePasswordDto);
    // } else {
